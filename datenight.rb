...
optparse = OptionParser.new do |opts|
  ...
  opts.banner = "Usage: datenight.rb [options] ideas-file"

  options[:weather] = 5
  opts.on( '-w', '--weather [0-9]', 'Current weather score.' ) do |val|
    options[:weather] = val
  end

  options[:time] = 5
  opts.on( '-t', '--time [0-9]', 'Time commitment score.' ) do |val|
    options[:time] = val
  end

  opts.on( '-l', '--list', 'List out the date-options in the file.' ) do
    command = "list"
  end
...
if command == "choose"
  puts $dates.choose(options[:weather], options[:time])
elsif command == "list"
  puts $dates
end